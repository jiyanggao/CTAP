...
def conv_1d_layer(name, bottom, kernel_size, stride, output_dim, padding='SAME',
    ...
    with tf.variable_scope(name):
        if weights_initializer is None and biases_initializer is None:
            ...
            print name+"input_dim"+str(input_dim)
    ...
    conv = tf.nn.conv1d(bottom, filters=weights,
        stride=stride, padding=padding)
    if bias_term:
        conv = tf.nn.bias_add(conv, biases)
    return conv

def conv_1d_relu_layer(name, bottom, kernel_size, stride, output_dim, padding='SAME',
                    bias_term=True, weights_initializer=None, biases_initializer=None):
    conv = conv_1d_layer(name, bottom, kernel_size, stride, output_dim, padding,
...
def pooling_layer_1d(name,bottom,kernel_size,stride):
    bottom=tf.reshape(bottom,[bottom.get_shape().as_list()[0],1,bottom.get_shape().as_list()[1],bottom.get_shape().as_list()[2]])
    pool = tf.nn.max_pool(bottom, ksize=[1, 1, kernel_size, 1],
        strides=[1, 1, stride, 1], padding='SAME', name=name)
    pool=tf.reshape(pool,[pool.get_shape().as_list()[0],pool.get_shape().as_list()[2],pool.get_shape().as_list()[3]])
    return pool
